	...
	<div class="flex-1 min-h-0 overflow-y-auto w-full pr-2">
		<div class={surahListClasses}>
			{#each quranMetaData.slice(1) as chapter (chapter.id)}
				...
				<button type="button" class="{individualCheckboxClasses} {isFavorite ? selectedRadioOrCheckboxClasses : ''}" aria-label={isFavorite ? `Remove ${chapter.transliteration} from favorites` : `Add ${chapter.transliteration} to favorites`} on:click={() => toggleFavoriteChapter(chapter.id)}>
