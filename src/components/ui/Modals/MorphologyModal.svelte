...
<Modal bind:open={$__morphologyModalVisible} id="morphologyModal" transitionParams={getModalTransition('bottom')} size="lg" class="rounded-b-none! md:rounded-3xl! max-h-[90vh] flex flex-col" bodyClass="p-6 flex flex-col min-h-0 overflow-hidden" position="bottom" center outsideclose>
	<div class="flex flex-row space-x-2 shrink-0">
		...
		<a href="/morphology?word={$__morphologyKey}" class="inline-flex mt-[-2px] mb-6 p-2 rounded-full items-center hover:bg-theme-accent/10 bg-theme-accent/5" on:click={() => window.umami.track('Full View Morphology Button')}>
			<ExternalLink size={4} />
		</a>
	</div>
