<script>
        ...
        import { playVerseAudio, showAudioModal, playButtonHandler, prepareVersesToPlay, toggleAudioPause } from '$utils/audioController';
        ...
        // Audio state of this particular verse
        $: isThisVerseActive = $__audioSettings.isPlaying && $__audioSettings.playingKey === key;
        $: isThisVersePaused = isThisVerseActive && $__audioSettings.isPaused;
        $: isThisVersePlaying = isThisVerseActive && !$__audioSettings.isPaused;
        $: playLabel = isThisVersePaused ? 'Resume' : isThisVersePlaying ? 'Pause' : 'Play';

        async function audioHandler(key) {
                // This verse is already loaded: just pause or resume it
                if (isThisVerseActive) return toggleAudioPause();
...
<div class="verseButtons flex flex-row justify-between">
        <div class="flex flex-row w-full space-x-2">
                ...
                <!-- other verse buttons -->
                <div class="flex flex-row space-x-2">
                        <!-- play/pause/resume verse button -->
                        <button on:click={() => audioHandler(key)} class={buttonClasses} aria-label={playLabel}>
                                <div>
                                        <svelte:component this={isThisVersePlaying ? Pause : Play} size={3.5} />
                                </div>
                        </button>
                        <Tooltip arrow={false} type="light" placement="top" class="z-30 hidden md:block font-normal">{playLabel}</Tooltip>
