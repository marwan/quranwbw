<script>
        ...
        import { resetAudioSettings, shouldKeepAudioOnNavigation } from '$utils/audioController';
        ...
        // except for a display change (to or from Mushaf) made while verse audio is playing
        $: if ($__currentPage || $__chapterNumber) {
                if (!shouldKeepAudioOnNavigation()) resetAudioSettings({ location: 'end' });
        }
